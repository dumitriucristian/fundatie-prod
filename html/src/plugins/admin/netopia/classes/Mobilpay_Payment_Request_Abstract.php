...
use \Admin\Netopia\Classes\Mobilpay_Payment_Request_Notify;
...
abstract class Mobilpay_Payment_Request_Abstract
{
	...
	public $type		= self::PAYMENT_TYPE_CARD;
	...
	abstract protected function _loadFromXml( $elem);
	...
	static public function factoryFromEncrypted($envKey, $encData, $privateKeyFilePath, $privateKeyPassword = null)
	{
		...
		$data = null;
		$result = @openssl_open($srcData, $data, $srcEnvKey,  $privateKey,self::CIPHER_ALGO);
		if($result === false)
		{
	...
	static protected function _factoryFromXml( $xmlDoc)
	{
	...
	protected function _parseFromXml( $elem)
	{
		$xmlAttr = $elem->attributes->getNamedItem('id');
		if($xmlAttr == null || strlen((string)$xmlAttr->nodeValue) == 0)
		{
			throw new \Exception('Mobilpay_Payment_Request_Card::_parseFromXml failed: empty order id', self::ERROR_LOAD_FROM_XML_ORDER_ID_ATTR_MISSING);
		}
		...
		if($elems->length != 1)
		{
			throw new \Exception('Mobilpay_Payment_Request_Card::loadFromXml failed: signature is missing', self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
		}
	...
	public function encrypt($x509FilePath)
	{
        ...
        $iv = '';
		$result 	= openssl_seal($srcData, $encData, $envKeys, $publicKeys, self::CIPHER_ALGO, $iv);
